    ...
    private void fitBestConstants(DTree[] ktrees, int[] leafs, GammaPass gp, Constraints cs) {
      ...
      for (int k = 0; k < _nclass; k++) {
        ...
        for (int i = 0; i < tree._len - leafs[k]; i++) {
          ...
          double gamma;
          if (useBounds) {
            double numerator = leafNode.splitRawPred();
            gamma = gp.calcGamma(k, i, numerator);
          } else {
            gamma = gp.gamma(k, i);
          }
...
//          Log.info(f + " vs " + y); //expect that the model predicts very negative values for 0 and very positive values for 1
  ...
  private static class GammaPass extends MRTask<GammaPass> {
    ...
    double gamma(int tree, int nid) {
      return calcGamma(tree, nid, _num[tree][nid]);
    }

    double calcGamma(int tree, int nid, double num) {
